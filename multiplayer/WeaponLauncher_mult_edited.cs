...
public class WeaponLauncher_mult_edited : NetworkBehaviour
{
	[HideInInspector]
	...
	private void Update ()
	{
		...
		{
			Debug.Log ("MAIN SHOT ----XXXX----");
			CmdShoot ();
		}

		if (!shoot_btn) 
		{
			shoot_btn=GameObject.Find("shoot").GetComponent<Button>();
			shoot_btn.onClick.AddListener(nothing);
		}
...
//						Debug.Log ("shoot 4x");
						...
						DamageBase_mult damangeBase = bullet.GetComponent<DamageBase_mult> ();
						if (damangeBase) {
							damangeBase.Owner = Owner;
							damangeBase.TargetTag = TargetTag;
						}
						WeaponBase_mult weaponBase = bullet.GetComponent<WeaponBase_mult> ();
						if (weaponBase) {
