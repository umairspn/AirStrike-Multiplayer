...
[RequireComponent(typeof(DamageManager_mult))]
[RequireComponent(typeof(WeaponController_mult))]


public class FlightSystem_mult : NetworkBehaviour
{
	Button boost;
	bool cooldown=false, coolup=false;
	...
	[HideInInspector]
	public DamageManager_mult DamageManage;
	[HideInInspector]
	public WeaponController_mult WeaponControl;// weapon system
	...
	void Start ()
	{
		...
		DamageManage = this.gameObject.GetComponent<DamageManager_mult> ();
		WeaponControl = this.gameObject.GetComponent<WeaponController_mult> ();
		mainRot = this.transform.rotation;
		GetComponent<Rigidbody>().mass = Mess;

		boost=GameObject.Find("boost").GetComponent<Button>();
		boost.onClick.AddListener(boost_click);
	}
	...
	void reset_cooldown()
	{
		...
		coolup = true;
		cooldown = false;
		Invoke ("show_boost", 10);
	}

	void show_boost()
	{
		boost.GetComponent<Image> ().fillAmount =1f;
	...
	void Update()
	{
		if (cooldown == true) 
		{
			boost.GetComponent<Image> ().fillAmount -= 0.005f;
		}
		if (coolup == true) 
		{
			boost.GetComponent<Image> ().fillAmount += 0.0010f;
		}
	}
