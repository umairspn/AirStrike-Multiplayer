...
public class Damage_mult : DamageBase_mult
{
    ...
    private void ExplosionDamage()
    {
        ...
        for (int i = 0; i < hitColliders.Length; i++)
        {
            ...
            if (hit.gameObject.GetComponent<DamageManager_mult>())
            {
                if (hit.gameObject.GetComponent<DamageManager_mult>())
                {
					hit.gameObject.GetComponent<DamageManager_mult>().RpcApplyDamage(Damage,Owner);
                }
            }
    ...
    {
        if (collision.gameObject.GetComponent<DamageManager_mult>())
        {
			collision.gameObject.GetComponent<DamageManager_mult>().RpcApplyDamage(Damage,Owner);
        }
    }
